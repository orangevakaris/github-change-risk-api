...
The analysis engine and HTTP interface are implemented, tested, and available as a rate-limited preview at [76.13.79.47.sslip.io](https://76.13.79.47.sslip.io). Per-file full reports are payable now through verified native-USDC or native-ETH transfers on Base.
...
The action writes the score, level, compact JSON report, and prefilled `checkout-url` to outputs and adds a readable report to the workflow summary. Its optional `fail-on` input can turn `low`, `medium`, or `high` risk levels into a deterministic CI failure. Supply the optional `payment-tx` input only after sending a confirmed Base USDC or ETH payment to request the per-file full report.
...
OpenAPI discovery document: `https://76.13.79.47.sslip.io/openapi.json`
